...
export default function NavigationSection(){
    ...
    <div className={` ${navFixed ? 'fixed w-full ': 'relative'} flex flex-col items-center top-0 fixed-0 z-10`} ref={navSection} >
<div className={`flex justify-center md:gap-10 text-center md:p-2 items-center bg-gray-700 w-full text-stone-900 font-semibold ${navShow}`}>
    ...
    <button className='md:bg-stone-500/40 pt-5 md:p-4 rounded md:hover:bg-red-500/30'  onClick={() => {pageNavigate(nav.refer)}} data-aos='zoom-in'>
    {nav.navCon}
    </button>
    </div>))}
